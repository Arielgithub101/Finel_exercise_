...
class Matrix:
    def __init__(self, row: int, col: int):
        self.matrix = [[4 for j in range(col)] for i in range(row)]

    def __getitem__(self, item:int):
        return self.matrix[item]

    def __add__(self, matrix_obj: 'Matrix'):
    ...
    def __sub__(self, matrix_obj: 'Matrix'):
        if len(matrix_obj.matrix) != len(self.matrix) or len(matrix_obj.matrix[0]) != len(self.matrix[0]):
    ...
    def compar_sum(self, matrix_obj: 'Matrix') -> bool:
        sum1: int = 0
        sum2: int = 0
        for i in range(len(self.matrix)):
            sum1 += self.sum_row(i)
    ...
    def get_multiplied_submatrix(self, row_start: int, row_end: int, col_start: int, col_end: int, scaler: int) -> list[
        List[int]]:
